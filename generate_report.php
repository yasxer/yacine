...
if (!class_exists('PHPMailer\PHPMailer\PHPMailer')) {
    die('<h2>Error: PHPMailer not found</h2><p>Please run <code>composer require phpmailer/phpmailer</code> in your terminal to install the missing library.</p>');
}
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    jsonResponse(false, 'Use the upload form.');
}

if (!isset($_FILES['xls_file']) || $_FILES['xls_file']['error'] !== UPLOAD_ERR_OK) {
    jsonResponse(false, 'File upload error. Please provide a valid .xls file.');
}

$uploadedName = $_FILES['xls_file']['name'];
$ext = strtolower(pathinfo($uploadedName, PATHINFO_EXTENSION));
if ($ext !== 'xls') {
    jsonResponse(false, 'Unsupported file type. Please upload a .xls file.');
}
...
try {
    ...
    jsonResponse(false, 'Could not read Excel file: ' . $e->getMessage());
}
...
// Get PDF content as string instead of streaming
...
try {
    ...
    $myEmail = 'ysxr.dev@gmail.com';
    $myAppPassword = 'feaa zuqd xhpw oidv'; // <-- Hna fin kayn l-ghalat
    ...
    if ($myEmail === 'votre_email@gmail.com' || $myAppPassword === 'votre_mot_de_passe_application') {
        throw new Exception('Server configuration required (Email/App Password).');
    }
    ...
    // Recipients
    $mail->setFrom($myEmail, 'Installment Report');
    $mail->addAddress($recipientEmail);

    // Attachments
    $mail->addStringAttachment($pdfContent, 'overdue_report.pdf');

    // Content
    $mail->isHTML(true);
    $mail->Subject = 'Overdue Installment Report - ' . date('Y-m-d');
    $mail->Body    = 'Here is the generated overdue report.';

    $mail->send();
    jsonResponse(true, 'Message has been sent to ' . $recipientEmail . ' successfully.');
} catch (Exception $e) {
    jsonResponse(false, "Message could not be sent. Mailer Error: {$mail->ErrorInfo} " . $e->getMessage());
}
exit;
